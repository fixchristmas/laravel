#!/usr/bin/env bash
# This is a simple task runner for Laravel, similar to supervisor.
#
...
do
    ...
    # Establish the maximum execution time for workers, in seconds.
    if [ -z "$WORKER_TTL" ]
    then
        workerttl=300
    else
        workerttl=$WORKER_TTL
    fi
    ...
    then
        ...
        sudo -u $appuser bash -c ". /opt/elasticbeanstalk/support/envvars ; /usr/bin/php -d max_execution_time=$workerttl /var/app/current/artisan queue:work --sleep=0"
    fi
